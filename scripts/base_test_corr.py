...
def save_output2(module, input, output):
     global activation,i, randCh
#     randCh = np.random.randint(output.shape[1])
#     pdb.set_trace();
 #    print(randCh[i])
     feat = output[:,randCh[i],:]
     activation[i] = feat
     i+=1
     return 
...
class Test:
    ...
    def get_performance(self, dataloader, trainloader, city):
        ...
        if self.bn and city != self.config.dataset:
            ...
            self.model.train()
            layr=0; first=1; global randCh; randCh={}
            for h in self.model.modules():
                if isinstance(h, nn.Conv2d): #Conv2d):
                   if first==1:
                     randCh[layr] = np.random.randint(h.out_channels)
                   layr+=1
                ...
                with torch.no_grad():
                    ...
                    for k in activation.keys():
                      ...
                      for row in range(0,activation[k].shape[1]):
                         actkrow = activation[k][:,row,:-1].reshape(-1)
                         pix[k] = np.hstack((pix[k],actkrow)) 
                         actkrow1 = activation[k][:,row,1:].reshape(-1)
