...
class Test:
    def __init__(self, model_path, config, bn, save_path, save_batch, sample_number, trial=100, cuda=False, num_layers=61, city_name = 'Vegas'):
        self.bn = bn
    ...
    def test(self, name, converged_model):
        ...
        for l in range(0,self.num_layers,1): 
            iter_model = copy.deepcopy(main_model)
            tA, tI, tIm = self.get_performance(self.target_loader[0], self.target_trainloader[0], self.target, l, iter_model)
            #pdb.set_trace()
            i_all.append(tI)

            name_f = '{}/{}_{}_adabn_k.pickle'.format(name,self.target, converged_model)
...
if __name__ == "__main__":
    ...
    parser.add_argument('city_name', default='Vegas', type = str)
    ...
    test = Test(fullpath, config, bn, save_path, save_batch, args.sample_number, cuda=args.cuda, num_layers = args.num_layers, city_name = args.city_name)
