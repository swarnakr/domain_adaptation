     ...
     if city=='Shanghai':
        for x in range(N):
            for y in range(N):
                ...
                activation_slicer = (slice(None), slice(int(x*activation_stride), int((x+1)*activation_stride)), slice(int(y*activation_stride), int((y+1)*activation_stride)))
...
i_a_filename = '/home/home1/swarnakr/main/DomainAdaptation/DA_viaBatchNorm/configs/ganDA/train_log/i_a_20200405_part_{}_of_5.pickle'
...
i_patches, a_patches, g_patches, c_patches, g_indices = getPatches(images, activations, gts, cities) #getPatches(*getImagesActivations())
